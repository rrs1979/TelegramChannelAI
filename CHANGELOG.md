...
### Security
- Copy buttons on the settings page no longer paste the field's value into their inline `onclick` — that put an env-controlled string inside a JS string inside an HTML attribute, and since the browser decodes the attribute before the JS is parsed, jinja's `&#39;` turned back into a quote that closed the string and ran whatever followed as script; the buttons now take the input's id and read the value off the element
...
- Pin the analytics Chart.js CDN bundle with a Subresource Integrity hash — the tag loaded `chart.js@4.4.7` straight off jsdelivr with no integrity check, so a tampered or MITM'd CDN response would have run arbitrary JS on the authenticated dashboard; the browser now drops the script unless its `sha384` matches
